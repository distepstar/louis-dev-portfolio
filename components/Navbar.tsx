...
export default function Navbar() {
  ...
  return (
    <header
      className={
        `min-w-full flex flex-col place-items-center z-10 fixed ` +
        (isOpen ? "pointer-events-auto" : "pointer-events-none")
      }
    >
      <DropDownTrail open={isOpen}>
        <div className="sm:h-26 h-20 flex flex-row justify-between place-items-center bg-slate-800 bg-opacity-70 rounded-2xl sm:w-[70em] w-[22em] sm:px-8 px-4 sm:my-12 my-10 shadow-lg">
          <h1
            className="sm:text-3xl text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-slate-200 to-cyan-400 blinking-cursor cursor-pointer"
          ...
          <Menu as="div" className="relative inline-block text-left">
            <div>
              ...
              >
                <Bars3BottomRightIcon
                  className="sm:h-6 sm:w-6 h-8 w-8 text-violet-200 hover:text-violet-100"
                  aria-hidden="true"
                />
              </Menu.Button>
            </div>
            ...
            >
              <Menu.Items className="absolute right-0 mt-2 sm:w-56 w-32 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <Menu.Item>
                  {({ active }) => (
                    <button
                      className={`${
